...
class DetectorTF2:
	...
	def DetectFromImage(self, img, img_name):
		...
		det_boxes = self.ExtractBBoxes(img_name, bboxes, bclasses, bscores, im_width, im_height)

		return det_boxes


	def ExtractBBoxes(self, img_name,bboxes, bclasses, bscores, im_width, im_height):
		bbox = []
		#print(bclasses.shape)
		#print(bscores.shape)
		for idx in range(len(bboxes)):
			if self.class_id is None or bclasses[idx] in self.class_id:
				if bscores[idx] >= self.Threshold:
					...
					bbox.append([img_name, x_min, x_max, y_min, y_max, class_label, float(bscores[idx])])
		return bbox
	...
	def DisplayDetections(self, image, boxes_list, det_time=None):
		...
		for idx in range(len(boxes_list)):
			x_min = boxes_list[idx][1]
			x_max = boxes_list[idx][2]
			y_min = boxes_list[idx][3]
			y_max = boxes_list[idx][4]
			score = str(np.round(boxes_list[idx][-1], 2))

			text =  score
			cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (0, 255, 0), 1)
# 			cv2.rectangle(img, (x_min, y_min - 20), (x_min, y_min), (255, 255, 255), -1)
			cv2.putText(img, text, (x_min + 5, y_min - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0), 1)
